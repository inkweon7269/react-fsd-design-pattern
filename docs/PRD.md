...
- 게시물 목록 조회, 상세 조회, 생성 기능을 React 프론트엔드에 구현
...
| 포함 | 미포함 |
|------|--------|
| 게시물 목록 조회 (페이지네이션) | 게시물 수정 (PATCH) |
| 게시물 상세 조회 | 게시물 삭제 (DELETE) |
| 게시물 생성 | 사용자 인증/인가 |
| FSD 레이어 구조 전체 셋업 | 검색/필터링 UI |
| shadcn/ui 기반 UI 구현 | 다크 모드 |
...
| FR-10 | 데이터 로딩 중 스켈레톤 UI를 표시한다 | P1 |
| FR-11 | 존재하지 않는 게시물 접근 시 에러 메시지를 표시한다 | P1 |

### 4.3 게시물 생성 페이지 (`/posts/create`)

| ID | 요구사항 | 우선순위 |
|----|----------|----------|
| FR-12 | 제목(Input), 내용(Textarea), 게시 여부(Switch) 입력 폼을 제공한다 | P0 |
| FR-13 | 제목은 필수이며 최대 200자 제한, 내용은 필수이다 | P0 |
| FR-14 | 유효성 검증 실패 시 각 필드 아래에 에러 메시지를 표시한다 | P0 |
| FR-15 | 생성 버튼 클릭 시 API를 호출하고, 성공 시 해당 게시물 상세 페이지로 이동한다 | P0 |
| FR-16 | API 호출 중 버튼을 비활성화하고 "Creating..." 텍스트를 표시한다 | P1 |
| FR-17 | 생성 성공 후 게시물 목록 캐시를 자동 무효화하여 최신 데이터를 반영한다 | P1 |
| FR-18 | API 에러 발생 시 폼 하단에 에러 메시지를 표시한다 | P1 |

### 4.4 공통 네비게이션

| ID | 요구사항 | 우선순위 |
|----|----------|----------|
| FR-19 | 상단 헤더에 앱 로고, "All Posts" 링크, "New Post" 버튼을 표시한다 | P0 |
| FR-20 | 루트 경로(`/`) 접근 시 `/posts`로 자동 리다이렉트한다 | P0 |
...
[Widgets] ← UI 합성, 이벤트 핸들링 위임
    ↓
[Features] ← mutation 훅 (useCreatePost)
...
- `features/post/` — 게시물 수정 폼, 게시물 삭제 기능 추가
