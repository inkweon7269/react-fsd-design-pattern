...
| 스킬 | 설명 | 커버 파일 패턴 |
|------|------|---------------|
| `verify-fsd` | FSD 아키텍처 규칙 준수 여부 검증 | `src/app/**`, `src/pages/**`, `src/widgets/**`, `src/features/**`, `src/entities/**`, `src/shared/**` |
...
| 디렉토리       | 파일                           |
| -------------- | ------------------------------ |
| src/entities   | `post/index.ts`, `post/api/`   |
| src/features   | `post/ui/CreatePostForm.tsx`   |
...
| 스킬        | 트리거 파일 (변경된 파일)      | 액션      |
| ----------- | ------------------------------ | --------- |
| verify-fsd  | `entities/post/index.ts`       | CHECK     |
...
| 스킬       | 갭 유형   | 상세                                                |
| ---------- | --------- | --------------------------------------------------- |
| verify-fsd | 파일 누락 | `src/entities/user/index.ts`가 Related Files에 없음 |
| verify-fsd | 새 패턴   | 새 slice가 검사되지 않는 규칙을 사용                |
```
...
- `verify-fsd` — 누락된 파일 참조 2개 추가, 탐지 패턴 업데이트
...
| -------------------------- | ----------------------------------- |
| ... 기존 항목 ...          |
| `src/entities/user/index.ts` | 새 entity slice public API |
```
...
2. **사용자에게 스킬 이름 확인** — `AskUserQuestion`을 사용합니다:
   ...
   **이름 규칙:**
   - 이름은 반드시 `verify-`로 시작해야 합니다 (예: `verify-tanstack`, `verify-shadcn`, `verify-testing`)
...
1. **Lock 파일 및 생성된 파일** — `package-lock.json`, `yarn.lock`, `pnpm-lock.yaml`, 자동 생성된 파일, 빌드 출력물은 스킬 커버리지가 불필요
